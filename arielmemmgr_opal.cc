...
MemoryManagerOpal::MemoryManagerOpal(ComponentId_t id, Params &params)
  ...
  while (isPortConnected(linkname)) {
    SST::Link *link =
        configureLink(linkname, latency,
                      new Event::Handler<MemoryManagerOpal>(
                          this, &MemoryManagerOpal::handleInterrupt));
    opalLink.push_back(link);
    numPorts++;
    linkname = linkprefix + std::to_string(numPorts);
  }

  std::string translatorstr =
      params.find<std::string>("translator", "ariel.MemoryManagerSimple");
  if (NULL != (temp_translator =
                   loadUserSubComponent<ArielMemoryManager>("translator"))) {
    output->verbose(CALL_INFO, 1, 0,
                    "Opal is using named subcomponent translator\n");
  } else {
    ...
    temp_translator = loadAnonymousSubComponent<ArielMemoryManager>(
        translatorstr, "translator", 0,
        ComponentInfo::SHARE_STATS | ComponentInfo::INSERT_STATS,
        translatorParams);
    if (NULL == temp_translator)
...
void MemoryManagerOpal::handleInterrupt(SST::Event *event) {
  OpalEvent *ev = dynamic_cast<OpalComponent::OpalEvent *>(
      event); // TODO can we static_cast instead?
  output->verbose(CALL_INFO, 4, 0,
                  "Core %" PRIu32 " handling opal interrupt event\n",
                  ev->getCoreId());
...
bool MemoryManagerOpal::allocateMalloc(const uint64_t size,
                                       const uint32_t level,
                                       const uint64_t addr, const uint64_t ip,
                                       const uint32_t thread) {
  OpalEvent *tse =
      new OpalEvent(OpalComponent::EventType::HINT, level, addr, size, thread);
...
void MemoryManagerOpal::printStats() { temp_translator->printStats(); }
