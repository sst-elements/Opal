...
#define OPAL_MEMNIC_ELI_PARAMS                                                 \
  MEMNICBASE_ELI_PARAMS,                                                       \
      ...
      {"min_packet_size",                                                      \
       "Size of packet with a payload (e.g., control message size)", "8B"}

  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      OpalMemNIC, "Opal", "OpalMemNIC", SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "MemNIC for Opal multi-node configurations",
  ...
  public:
    ...
    OpalInitMemRtrEvent() = default;
    OpalInitMemRtrEvent(EndpointInfo info, uint32_t node)
        : InitMemRtrEvent(info), node(node) {}

    Event *clone() override {
      auto *imre = new OpalInitMemRtrEvent(*this);
...
protected:
  ...
  void processInitMemRtrEvent(
      MemHierarchy::MemNICBase::InitMemRtrEvent *ev) override;

private:
  void build(Params &params); // Temporary function called by both constructors
                              // until both are no longer required
