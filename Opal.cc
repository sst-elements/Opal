...
#define OPAL_VERBOSE(LEVEL, OUTPUT)                                            \
  if (verbosity >= (LEVEL))                                                    \
  OUTPUT
...
Opal::Opal(SST::ComponentId_t id, SST::Params &params) : Component(id) {
  ...
  output = new SST::Output("OpalComponent[@f:@l:@p] ", verbosity, 0,
                           SST::Output::STDOUT);
  ...
  std::cerr << getName().c_str()
            << "Number of Shared Memory Pools: " << num_shared_mempools << endl;
  ...
  for (uint32_t i = 0; i < num_shared_mempools; i++) {
    ...
    std::cerr << getName().c_str() << "Configuring Shared " << buffer
              << std::endl;
    shared_mem_size += memPoolParams.find<uint64_t>("size", 0);
    memset(buffer, 0, 256);
    sprintf(buffer, "globalMemCntrLink%" PRIu32, i);
    sharedMemoryInfo[i]->link = configureLink(
        buffer, "1ns",
        new Event::Handler<MemoryPrivateInfo>(
            (sharedMemoryInfo[i]), &MemoryPrivateInfo::handleRequest));
  }
  ...
  for (uint32_t i = 0; i < num_nodes; i++) {
    ...
    for (uint32_t j = 0; j < nodeInfo[i]->cores; j++) {
      ...
      nodeInfo[i]->coreInfo[j].coreLink = configureLink(
          buffer, "1ns",
          new Event::Handler<CorePrivateInfo>((&nodeInfo[i]->coreInfo[j]),
                                              &CorePrivateInfo::handleRequest));
      memset(buffer, 0, 256);
      sprintf(buffer, "mmuLink%" PRIu32, num_cores);
      nodeInfo[i]->coreInfo[j].mmuLink = configureLink(
          buffer, "1ns",
          new Event::Handler<CorePrivateInfo>((&nodeInfo[i]->coreInfo[j]),
                                              &CorePrivateInfo::handleRequest));
      num_cores++;
    ...
    for (uint32_t j = 0; j < nodeInfo[i]->memory_cntrls; j++) {
      ...
      nodeInfo[i]->memCntrlInfo[j].link =
          configureLink(buffer, "1ns",
                        new Event::Handler<MemoryPrivateInfo>(
                            (&nodeInfo[i]->memCntrlInfo[j]),
                            &MemoryPrivateInfo::handleRequest));
      num_memCntrls++;
    }

    char *subID = (char *)malloc(sizeof(char) * 32);
    sprintf(subID, "%" PRIu32, i);
    nodeInfo[i]->statLocalMemUsage =
        registerStatistic<uint64_t>("local_mem_usage", subID);
    nodeInfo[i]->statSharedMemUsage =
        registerStatistic<uint64_t>("shared_mem_usage", subID);
    free(subID);
  }
...
void Opal::setNextMemPool(int node, int /*fault_level*/) {
  ...
  case 3:
    ...
    nodeInfo[node]->nextallocmem =
        (nodeInfo[node]->nextallocmem + 1) %
        (shared_mem_size / nodeInfo[node]->memory_size + 1);
    nodeInfo[node]->allocatedmempool =
        nodeInfo[node]->nextallocmem
            ? (nodeInfo[node]->allocatedmempool + 1) % (num_shared_mempools + 1)
                  ? (nodeInfo[node]->allocatedmempool + 1) %
                        (num_shared_mempools + 1)
                  : 1
            : 0;
  ...
  case 2:
    ...
    nodeInfo[node]->nextallocmem =
        (nodeInfo[node]->nextallocmem + 1) % (num_shared_mempools + 1);
    nodeInfo[node]->allocatedmempool = nodeInfo[node]->nextallocmem;
    break;
...
void Opal::processHint(int node, int fileId, uint64_t vAddress, int size) {
  ...
  auto fileIdHint = opalBase->mmapFileIdHints.find(fileId);
  ...
  if (fileIdHint != opalBase->mmapFileIdHints.end()) {
    ...
    if (it1 != it->end()) {
      std::cerr << "Memory is already allocated for fileId: "
                << fileIdHint->first << " in the same node" << std::endl;
    } else {
      int owner_node = it->back();
      if (nodeInfo[owner_node]->page_size != nodeInfo[node]->page_size)
        output->fatal(CALL_INFO, -1,
                      "Opal: Page sizes of the owner node which reserved space "
                      "with fileId: %d and the requesting node are not same\n",
                      fileId);

      it->push_back(node);
      //(fileIdHint->second).first = it;
      nodeInfo[node]->reservedSpace.insert(std::make_pair(
          vAddress / 4096,
          std::make_pair(
              fileId,
              std::make_pair(ceil(size / (nodeInfo[node]->page_size)), 0))));
    }
  } else {
    auto *it = new std::vector<int>;
    auto *pa = new std::vector<uint64_t>;

    it->push_back(node);
    opalBase->mmapFileIdHints.insert(
        std::make_pair(fileId, std::make_pair(it, pa)));
    nodeInfo[node]->reservedSpace.insert(std::make_pair(
        vAddress / 4096,
        std::make_pair(
            fileId,
...
REQRESPONSE Opal::isAddressReserved(int node, uint64_t vAddress) {
  ...
  for (auto it = (nodeInfo[node]->reservedSpace).begin();
       it != (nodeInfo[node]->reservedSpace).end(); ++it) {
    uint64_t reservedVAddress = it->first;
    int pages_reserved = (it->second).second.first;
    if (reservedVAddress <= vAddress &&
        vAddress <
            reservedVAddress + pages_reserved * nodeInfo[node]->page_size) {
      response.status = 1;
...
REQRESPONSE Opal::allocateSharedMemory(int node, int /*coreId*/,
                                       uint64_t /*vAddress*/, int fault_level,
                                       int pages) {
  REQRESPONSE response;
  response.status = 0;
      ...
      if (sharedMemoryInfo[i]->pool->available_frames >= pages) {
        ...
        for (int j = 0; j < pages; j++) {
          ...
          if (!response.status)
            output->fatal(
                CALL_INFO, -1,
                "Opal: Allocating shared memory. This should never happen\n");
...
REQRESPONSE Opal::allocateLocalMemory(int node, int coreId, uint64_t vAddress,
                                      int fault_level, int pages) {
  ...
  if (nodeInfo[node]->pool->available_frames >= pages) {
    ...
    for (int i = 0; i < pages; i++) {
      ...
      if (!response.status)
        output->fatal(
            CALL_INFO, -1,
            "Opal: Allocating local memory. This should never happen\n");
    ...
    OPAL_VERBOSE(8, output->verbose(
                        CALL_INFO, 8, 0,
                        "Node%" PRIu32 " Local Memory is drained out\n", node));
...
REQRESPONSE Opal::allocateFromReservedMemory(int node,
                                             uint64_t reserved_vAddress,
                                             uint64_t vAddress, int pages) {
  REQRESPONSE response;
  response.status = 0;

  int fileID = nodeInfo[node]->reservedSpace[reserved_vAddress].first;
  int pages_reserved =
      nodeInfo[node]->reservedSpace[reserved_vAddress].second.first;
  int pages_used =
      nodeInfo[node]->reservedSpace[reserved_vAddress].second.second;

  std::vector<uint64_t> *reserved_pAddress =
      opalBase->mmapFileIdHints[fileID].second;

  // Allocate all the pages. TODO: pages can be reserved on demand instead of
  // allocating all the pages at a time. But what if the memory is drained out.
  if (reserved_pAddress->empty()) {
      ...
      if (sharedMemoryInfo[i]->pool->available_frames >= pages) {
        ...
        for (int j = 0; j < pages_reserved; j++) {
          ...
          if (!response.status)
            output->fatal(
                CALL_INFO, -1,
                "Opal: Allocating reserved memory. This should never happen\n");
        }
    ...
    if (!response.status)
      output->fatal(
          CALL_INFO, -1,
          "Opal: memory not available to allocate memory for file ID: %d \n",
          fileID);
  }
  ...
  if (pages_used + pages <= pages_reserved) {
    ...
    output->fatal(
        CALL_INFO, -1,
        "Opal: address :%llu requested with fileId:%d has no space left\n",
        vAddress, fileID);
  }
...
bool Opal::processRequest(int node, int coreId, uint64_t vAddress,
                          int fault_level, int size) {
  ...
  // If multiple pages are requested how are the physical addresses sent to the
  // requester as in future sue to opal parallelization continuous addresses
  // cannot be allocated
  if (pages != 1)
    output->fatal(
        CALL_INFO, -1,
        "Opal: currently opal does not support multiple page allocations\n");

  // if the page fault request is for CR3 register allocate the memory from
  // local memory
  if (4 == fault_level)
    ...
    if (response.status)
      response =
          allocateFromReservedMemory(node, response.address, vAddress, pages);

    else {
      if (!nodeInfo[node]->allocatedmempool) {
        response =
            allocateLocalMemory(node, coreId, vAddress, fault_level, pages);
        // std::cerr << getName() << " Node: " << node << " core " << coreId <<
        // " response page address: " << vAddress << " allocated local address: "
        // << response.address << " pages: "<< pages << " level: " << fault_level
        // << std::endl;
      } else {
        response =
            allocateSharedMemory(node, coreId, vAddress, fault_level, pages);
        // std::cerr << getName() << " Node: " << node << " core " << coreId <<
        // " response page address: " << vAddress << " allocated shared address:
        // " << response.address << " pages: " << " level: " << fault_level <<
        // std::endl;
      }
    }
  }
  ...
  if (response.status) {
    auto *tse = new OpalEvent(EventType::RESPONSE);
    tse->setResp(vAddress, response.address,
                 response.pages * nodeInfo[node]->page_size);
  ...
  while (!opalBase->requestQ.empty()) {
    if (inst_served < max_inst) {
      ...
      case SST::OpalComponent::EventType::HINT: {
        std::cerr << getName().c_str() << " node: " << ev->getNodeId()
                  << " core: " << ev->getCoreId()
                  << " request page address: " << ev->getAddress() << " hint"
                  << std::endl;
      } break;

      case SST::OpalComponent::EventType::MMAP: {
        OPAL_VERBOSE(8, output->verbose(CALL_INFO, 8, 0,
                                        "Node%" PRIu32
                                        " Opal has received an MMAP CALL\n",
                                        ev->getNodeId()));
        std::cerr << "MLM mmap(" << ev->getFileId() << ") : level "
                  << ev->getHint() << " Starting address is " << std::hex
                  << ev->getAddress();
        std::cerr << std::dec << " Size: " << ev->getSize();
        std::cerr << " Ending address is " << std::hex
                  << ev->getAddress() + ev->getSize() - 1;
        std::cerr << std::dec << std::endl;
        // size should be in the multiple of page size (4096) from ariel core
        // processHint(ev->getNodeId(), ev->getFileId(), ev->getAddress(),
        // ev->getSize());
      } break;
      ...
      case SST::OpalComponent::EventType::UNMAP: {
        std::cerr << getName().c_str() << " node: " << ev->getNodeId()
                  << " core: " << ev->getCoreId()
                  << " request page address: " << ev->getAddress() << " unmap"
                  << std::endl;
        OPAL_VERBOSE(8, output->verbose(CALL_INFO, 8, 0,
                                        "Node%" PRIu32
                                        " Opal has received an UNMAP CALL\n",
                                        ev->getNodeId()));
      } break;

      case SST::OpalComponent::EventType::REQUEST: {
        removeEvent =
            processRequest(ev->getNodeId(), ev->getCoreId(), ev->getAddress(),
                           ev->getFaultLevel(), ev->getSize());
      } break;
      ...
      output->verbose(CALL_INFO, 2, 0,
                      "%s, number of requests served has reached maximum width "
