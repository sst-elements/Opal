...
public:
  ...
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      MemoryManagerOpal, "Opal", "MemoryManagerOpal",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "Memory manager which uses the Opal memory allocation component",
  ...
  SST_ELI_DOCUMENT_PARAMS(
      ...
      {"translator",
       "(temporary) translation memory manager to actually translate addresses "
       "for now",
       "MemoryManagerSimple"})

  SST_ELI_DOCUMENT_PORTS({"opal_link_%(corecound)d",
                          "Each core's link to the Opal memory manager",
                          {"Opal.OpalEvent"}})

  SST_ELI_DOCUMENT_SUBCOMPONENT_SLOTS(
      {"translator",
       "Temporarily, which memory manager to use to translate addresses",
       "SST::Ariel::ArielMemoryManager"})
  ...
  bool allocateMMAP(const uint64_t size, const uint32_t level,
                    const uint64_t virtualAddress,
                    const uint64_t instructionPointer, const uint32_t file,
...
private:
  ArielMemoryManager *temp_translator; // Temporary while Opal still uses
                                       // Ariel's built-in translator
