...
OpalMemNIC::OpalMemNIC(ComponentId_t id, Params &params)
    : SST::MemHierarchy::MemNICBase(id, params) {
  build(params);
}
...
void OpalMemNIC::build(Params &params) {
  ...
  link_control = loadUserSubComponent<SST::Interfaces::SimpleNetwork>(
      "linkcontrol", ComponentInfo::SHARE_NONE, 1);
  if (!link_control) {
    Params lcparams;
    lcparams.insert("port_name", "port");
    lcparams.insert("link_bw",
                    params.find<std::string>("network_bw", "80GiB/s"));
    lcparams.insert("in_buf_size", params.find<std::string>(
                                       "network_input_buffer_size", "1KiB"));
    lcparams.insert("out_buf_size", params.find<std::string>(
                                        "network_output_buffer_size", "1KiB"));
    std::string lcSub =
        params.find<std::string>("linkcontrol", "merlin.linkcontrol");
    link_control = loadAnonymousSubComponent<SST::Interfaces::SimpleNetwork>(
        lcSub, "linkcontrol", 0,
        ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, lcparams, 1);
  }
  link_control->setNotifyOnReceive(
      new SST::Interfaces::SimpleNetwork::Handler<OpalMemNIC>(
          this, &OpalMemNIC::recvNotify));
...
void OpalMemNIC::processInitMemRtrEvent(
    MemHierarchy::MemNICBase::InitMemRtrEvent *ev) {
  auto *imre = static_cast<OpalInitMemRtrEvent *>(ev);
  dbg.debug(_L10_,
            "%s (OpalMemNIC) received imre. Name: %s, Addr: %" PRIu64
            ", ID: %" PRIu32 ", start: %" PRIu64 ", end: %" PRIu64
            ", size: %" PRIu64 ", step: %" PRIu64 ", node: %" PRIu32 "\n",
            getName().c_str(), imre->info.name.c_str(), imre->info.addr,
            imre->info.id, imre->info.region.start, imre->info.region.end,
            imre->info.region.interleaveSize, imre->info.region.interleaveStep,
            imre->node);

  if (sourceIDs.find(imre->info.id) !=
      sourceIDs.end()) { // From one of our source groups
    dbg.debug(_L10_, "\tAdding to sourceEndpointInfo. %zu sources found\n",
              sourceEndpointInfo.size());
    addSource(imre->info);
  } else if (destIDs.find(imre->info.id) !=
             destIDs.end()) { // From one of our dest groups
    // Filter by node if sharedmem is enabled
    if (enable) {
      if (imre->node == node || imre->node == 9999) {
        dbg.debug(_L10_,
                  "\tAdding to destEndpointInfo. %zu destinations found\n",
...
std::string OpalMemNIC::findTargetDestination(MemHierarchy::Addr addr) {
  for (const auto &it : destEndpointInfo) {
    if (it.region.contains(addr))
      return it.name;
  }
  ...
  if (enable && localMemSize) {
    ...
    for (const auto &it : destEndpointInfo) {
      if (it.region.contains(tempAddr))
        return it.name;
    }
  }
  ...
  /* Build error string */
  stringstream error;
  error << getName() + " (OpalMemNIC) cannot find a destination for address "
        << addr << endl;
