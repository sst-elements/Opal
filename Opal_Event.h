...
private:
  OpalEvent() = default; // For serialization

  EventType ev;
  uint64_t address;
  uint64_t paddress;
  int faultLevel;
  int size; // to redcue packet size, size valiable we used size variable for
            // multiple pusrposes. 1. size of the page fault. 2. shoowdownID. 3.
            // local and global page reference
  uint32_t nodeId;
  uint32_t coreId;
  MemType memType;
  int hint;
...
public:
  ...
  OpalEvent(EventType y, const uint32_t level, const uint64_t virtualAddress,
            const uint64_t size_, const uint32_t thread)
      : SST::Event() {
    ev = y;
    memType = SST::OpalComponent::MemType::LOCAL;
    invalidate = false;
